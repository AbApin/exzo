...
function Form({ label, handleClick, firebaseError }: FormProps) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button
        type="submit"
        className="block w-full text-white text-[11px] leading-[18px] text-center font-[raleway] uppercase font-bold relative cursor-pointer bg-[#b8cd06] py-[15px] px-[30px] border-0 rounded-[26px] mt-[20px]">
        {label}
      </button>

      {/* Local validation error */}
      {error && (
        <p className="text-red-800 text-center text-[11px] leading-[18px] font-[raleway] uppercase font-bold mt-[20px]">
          {error}
        </p>
      )}
