...
.product.column .productPrices {
 ...
 ? 'relative wrap group' : 'product column group'
  return (
    <div className={`flex bg-white cursor-pointer border-[1px] border-[#f7f7f7] px-[30px] py-[25px] relative ${showVersion === 'wrap' ? 'flex-col w-[33.333%]' : 'flex-row w-full'}`}>
      <div className={`${showVersion === 'wrap' ? 'order-3' : 'order-0'}`"font-[raleway] uppercase leading-[18px]">
        <p className="text-[#b8cd06] text-[11px]">{category}</p>
        <p className="text-[#343434] text-[13px]">{title}</p>
      </div>
      <div className={`${showVersion === 'wrap' ? 'order-1' : 'order-0'} w-[200px] h-[200px] mt-[10px] relative`}>
        ...
        <div className="hidden group-hover:flex absolute top-0 left-0 w-full h-full items-center justify-center bg-[rgba(255,255,255,0.9)] animate-fadeIn transition-all duration-300">
          <div className="flex flex-col gap-[10px]">
            <Link
              to={`/products/${id}`}
              className="inline-block text-white font-[raleway] text-center text-[11px] leading-[18px] font-bold rounded-[25px] shadow-[logout] uppercase bg-[#343434] px-[20px] py-[10px] cursor-pointer">
              Learn more
            </Link>
            <button
              onClick={() => dispatch(addToCart({ product }))}
              className="inline-block text-white font-[raleway] text-center text-[11px] leading-[18px] font-bold rounded-[25px] shadow-[logout] uppercase bg-[#b8cd06] border-0 px-[20px] py-[10px] cursor-pointer">
              Add to cart
            </button>
          </div>
        </div>
      </div>
      <div className={`${showVersion === 'wrap' ? 'order-3' : 'order-0'} flex items-center gap-[10px] mt-[10px] text-[16px] leading-[18px]`}>
      ...
      <div
        ...
        } flex items-center justify-center absolute r-[20px] top-[20px]  border-[1px] border-[#555555] rounded-[50%] cursor-pointer hover:bg-[#b8cd06] transition-all duration-300 group`}
        ...
        <svg width="21" height="19" viewBox="0 0 21 19" fill="#000">
          <path
            ...
            className="fill-current transition-colors duration-300 group-hover:fill-white"
          />
        </svg>
      </div>
    </div>
