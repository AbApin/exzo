...
const Product = ({ product }: { product: ProductType }) => {
  ...
  return (
    <div
      className={`flex w-full md: bg-white cursor-pointer border-[1px] border-[#f7f7f7] px-[30px] py-[25px] relative parent-group group ${
        showVersion === 'wrap' ? 'flex-col w-[33.333%]' : 'flex-row w-full'
      }`}>
      <div
        className={`${
          showVersion === 'wrap' ? 'order-2' : 'order-0'
      ...
      <div
        className={`${
          showVersion === 'wrap' ? 'order-1' : 'order-0'
      ...
      <div
        className={`${
          showVersion === 'wrap' ? 'order-3' : 'order-0'
      ...
      <div
        ...
        } flex items-center justify-center absolute right-[20px] top-[20px]  border-[1px] border-[#555555] rounded-[50%] cursor-pointer hover:bg-[#b8cd06] transition-all duration-300 group child-group`}
        onClick={() => handleClickFavouriteIcon(id)}>
        <svg width="21" height="19" viewBox="0 0 21 19" className="fill-[#555555]">
          <path
            ...
            className="transition-colors duration-300 fill-[#555555] child-group-hover:fill-white"
          />
        </svg>
      </div>
    </div>
