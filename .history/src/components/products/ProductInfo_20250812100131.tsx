          ...
          <div className="flex justify-between">
            <div className="w-1/2">
              <div className="w-[300px] h-[300px] m-auto">
                <img src={selectedImage} className="block max-w-full m-auto" alt="" />
              </div>
              <div className="flex items-center gap-[20px] mt-[20px]">
                {product.images.map((image) => {
                  return (
                    <div key={image} className="w-[30%] cursor-pointer transition-all duration-300">
            ...
            <div className="w-1/2">
              <p className="text-[#555] text-[14px] leading-[22px] uppercase">{product.category}</p>
              <p className="text-[#343434] text-[30px] leading-[34px] font-black font-[raleway] uppercase">
                {product.title}
              </p>
              <div className="flex items-center ju gap-[50px] mt-[20px]">
                <p className="text-[#555] text-[18px] leading-[24px] uppercase">
                  PRICE: <span className="text-[#b8cd06]">${discountPrice}</span>
                </p>
                <p className="text-[#888] text-[13px] leading-[20px]">
                  {product.reviews ? product.reviews.length : 0} Reviews
                </p>
              </div>
              <p className="text-[#888] text-[14px] leading-[22px] uppercase mt-[20px]">
                AVAILABLE: {product.availabilityStatus}
              </p>
              <p className="text-[#888] text-[14px] leading-[22px] mt-[20px]">
                {product.description}
              </p>
              <div className="flex items-center gap-[30px] mt-[30px]">
                ...
                <div className="block" onClick={() => handleClickFavouriteIcon()}>
                  <div
                    className={`${
                      haveInFavourites ? 'bg-[#b8cd06]' : 'bg-transparent'
                    } flex items-center justify-center  p-1 border-[1px] border-[#555555] rounded-[50%] cursor-pointer hover:bg-[#b8cd06] transition-all duration-300 group child-group`}>
