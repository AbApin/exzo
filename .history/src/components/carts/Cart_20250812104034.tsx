...
function Cart({ cart }: Props) {
  ...
  return (
    <div className="flex items-center justify-between py-[25px] px-[10px] border-[1px] border-[#eee]">
      <div className="flex items-center gap-[10px]">
        <div className="cartImg">
          <img src={thumbnail} className="block w-[85px] h-[85px] sh" alt={title} />
        </div>
        <p className="cartTitle">{title}</p>
      </div>
      <div className="cartRight">
        <p className="cartPrice">${discountPrice}</p>
        ...
        <p className="cartTotalPrice">${cartTotalPrice}</p>
