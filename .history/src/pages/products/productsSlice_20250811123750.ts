...
export const addFavourite = createAsyncThunk<FavouriteProductType, { product: FavouriteProductType }>(
  'products/addFavourite',
  async ({ product }: { product: FavouriteProductType }, { dispatch }) => {
...
export const cartCountMinus = createAsyncThunk<string, { cart: CartType }>(
  ...
  async ({ cart }, { dispatch }) => {
    ...
    const response = await fetch(`http://localhost:5000/carts/${cart.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
